...
test(
    ...
    async (t) => {
        t.true(await contains([1, 2, 3, 4], 2));

        const o = {};
        t.true(await contains([1, o, {}, NaN], o));

        t.true(await contains([-0, 4], 0));

        t.true(await contains([1, 2, NaN, "banana"], NaN));

        t.true(await contains([{}, "foo", 12], {}));
    },
);
